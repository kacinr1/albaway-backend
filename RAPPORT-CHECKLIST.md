...
| 15 | Temps de réponse (Render) | ✅/⚠️ | `/health` en place. Webhook Stripe **idempotent vérifié** (garde `payment_status !== 'paid'`). **Keep-alive ajouté** : GitHub Actions `.github/workflows/keepalive.yml` ping `/health` toutes les ~10 min (s'active seul au merge, tolérant au cold start via `--retry`) → garde le web Render **et** la DB Supabase au chaud, et alerte si le site tombe. **Reste (côté toi)** : **passer au tier payant Render avant la haute saison** (garantie zéro cold start pendant un paiement) ; option UptimeRobot en doublon. |
...
1. ~~**Keep-alive Render**~~ ✅ **FAIT (code)** — GitHub Actions `.github/workflows/keepalive.yml` ping `/health` toutes les ~10 min (warm web + DB, alerte si down). **Reste côté toi** : bascule tier payant Render avant la saison (cold start pendant paiement = risque) ; option UptimeRobot en doublon.
